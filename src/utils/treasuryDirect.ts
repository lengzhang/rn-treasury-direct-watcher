...
export async function getSecuritiesByType(
  type: SECURITY_TYPES_TYPE
): Promise<Security[]> {
...
export function parseOriginalSecurityTerm(term: string) {
  const obj = term
    ...
    .reduce<{ Year?: number; Month?: number; Week?: number; Day?: number }>(
      (acc, [num, unit]) => {
        if (unit === "Year") acc.Year = parseInt(num);
        else if (unit === "Month") acc.Month = parseInt(num);
        else if (unit === "Week") acc.Week = parseInt(num);
        else if (unit === "Day") acc.Day = parseInt(num);
        return acc;
      },
      {}
    );
